...
Block::Block(void) {
	for (size_t i = 0; BLOCKS_S[i].blockName != ""; ++i)
		_block[BLOCKS_S[i].blockName] = BLOCKS_S[i].directiveSet;
}
...
/* BLOCKS */
blockType_e	Block::dispatch(Token &token) {
	blocks_it	it = _block.find(token.getWord());
	blocks_it	end = _block.end();
	if (it == end) {
		std::ostringstream oss;
		oss << RED " >>>>>>>>>> Unknown block: " << token.getWord() << RENDL;
		throw (std::runtime_error(oss.str()));
	}
	std::cout << BLU "BLOCK: " GRN << token.getWord() << RENDL;
	return (it->second);
