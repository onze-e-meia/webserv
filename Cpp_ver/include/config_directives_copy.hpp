...
#define DIRECTIVE(name) { #name, &Directive::name }
...
typedef std::map<const std::string, int>	blocks_map;
typedef blocks_map::const_iterator			blocks_it;
...
public:
	...
	blockType_e	dispatch(Token &token);

	// handler_t		directives(Token &token);
	// handler_t		http(Token &token);
	// handler_t		server(Token &token);
	// handler_t		location(Token &token);
}; // namespace Block
